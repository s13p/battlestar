...
define([
    ...
    var
             ...
             */
            RENDER_QUEUE_NONE_BIT = 0,
            RENDER_QUEUE_FRONT_BIT = 1,
            RENDER_QUEUE_DISTANCE_BIT = 2,
            // ----------------------------------------------------------------------
            // constants
            /**
             * @type String
    ...
    RenderableObject.prototype.getRenderQueueBits = function () {
        return this._visible ? RENDER_QUEUE_FRONT_BIT : RENDER_QUEUE_NONE_BIT;
    };
    /**
    ...
    RenderableObject3D.prototype.getRenderQueueBits = function (camera, parentQueueBits) {
        var result = RENDER_QUEUE_NONE_BIT, baseMatrix, size;
     ...
     * @param {ParameterizedMesh} [parameterSource] If given, this mesh will share its parameters with the passed mesh
     */
    function ParameterizedMesh(model, shader, textures, positionMatrix, orientationMatrix, scalingMatrix, wireframe, lod, smallestSizeWhenDrawn, parameterArrayDeclarations, parameterSource) {
        ...
        if (model) {
            this.init(model, shader, textures, positionMatrix, orientationMatrix, scalingMatrix, wireframe, lod, smallestSizeWhenDrawn, parameterArrayDeclarations, parameterSource);
        }
    }
     ...
     * @param {ParameterizedMesh} [parameterSource] If given, this mesh will share its parameters with the passed mesh
     */
    ParameterizedMesh.prototype.init = function (model, shader, textures, positionMatrix, orientationMatrix, scalingMatrix, wireframe, lod, smallestSizeWhenDrawn, parameterArrayDeclarations, parameterSource) {
        var i, uniformArrayName, uniformArrayLength, parameterArrayNames;
        ...
        for (i = 0; i < parameterArrayNames.length; i++) {
            ...
            if (uniformArrayLength > 0) {
                if (parameterSource) {
                    this._parameterArrays[parameterArrayNames[i]] = parameterSource._parameterArrays[parameterArrayNames[i]];
                } else {
                ...
                this.setUniformValueFunction(parameterArrayNames[i], this.createGetParameterArrayFunction(this._parameterArrays[parameterArrayNames[i]]));
            } else {
    ...
    /**
     * Returns a function that returns the passed parameter array.
     * @param {Float32Array} array
     * @returns {Function}
     */
    ParameterizedMesh.prototype.createGetParameterArrayFunction = function (array) {
        return function () {
            return array;
        };
    };
    ...
    return {
        RENDER_QUEUE_FRONT_BIT: RENDER_QUEUE_FRONT_BIT,
        RENDER_QUEUE_DISTANCE_BIT: RENDER_QUEUE_DISTANCE_BIT,
